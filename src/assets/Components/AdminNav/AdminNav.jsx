	...
	useEffect(() => {
		const unsubscribe = onAuthStateChanged(auth, async (user) => {
			...
			if (user) {
				...
				try {
					...
					if (snapshot.exists()) {
						...
						if (userEntry) {
							...
							// Check initial status - if suspended, logout immediately
							if (userDataObj.status !== 'Active') {
								await signOut(auth);
								alert('Your account status has been changed. You have been logged out.');
								navigate("/admin");
								return;
							}
							...
							const userRef = ref(db, `users/${userEntry[0]}`);
							statusListenerRef.current = userRef;
							
							onValue(userRef, async (snapshot) => {
								if (snapshot.exists()) {
									const updatedUserData = snapshot.val();
									const currentStatus = updatedUserData.status;
									
									// If status changed to Suspended or inactive, logout
									if (currentStatus !== 'Active') {
										// Unsubscribe before logging out to prevent loops
										if (statusListenerRef.current) {
											off(statusListenerRef.current);
											statusListenerRef.current = null;
										}
										await signOut(auth);
										const statusMessage = currentStatus === 'Suspended'
											? 'Your account has been suspended. You have been logged out.'
											: 'Your account status has been changed. You have been logged out.';
										alert(statusMessage);
										navigate("/admin");
									}
								}
				...
				setUserData(null);
				// Clean up status listener when user logs out
				if (statusListenerRef.current) {
					off(statusListenerRef.current);
					statusListenerRef.current = null;
				}
